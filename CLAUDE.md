...
`template.html` 만 고친다. 종이·잉크·형광펜 팔레트, Gowun Dodum/Gowun Batang/Gaegu/IBM Plex Mono 글꼴, 왼쪽 링 제본, 오른쪽 인덱스 탭(표지·차례·인강·문제)이 정체성이다. 탭 이름은 하는 일이 드러나게. 진행 상태 표시(진도)·자신감 도장·페이지 내 AI 기능은 사용자가 뺀 것이니 다시 넣지 않는다.
